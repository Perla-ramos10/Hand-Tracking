...
texto = ""
ultimo_tiempo = 0
retardo = 2  # segundos entre teclas
...
def dibujar_teclado(frame, seleccionada=None):
    for i, fila in enumerate(teclado):
        for j, tecla in enumerate(fila):
            x, y = posiciones[i][j]
            color = (0, 255, 0) if seleccionada == (i, j) else (0, 0, 0)
...
while True:
    ...
    if resultado.multi_hand_landmarks:
        for hand in resultado.multi_hand_landmarks:
            ...
            # Gesto para borrar todo si el dedo está doblado
            indice = hand.landmark[8]
            base = hand.landmark[5]
            distancia = ((indice.x - base.x) ** 2 + (indice.y - base.y) ** 2) ** 0.5
            tiempo_actual = time.time()
            if distancia < 0.03 and tiempo_actual - ultimo_gesto > 2:
